...
function styles() {
  ...
  return gulp.src('simple-modal.scss')
    .pipe(plumber({errorHandler: onError}))
    .pipe(sass({outputStyle: 'expanded'})) /* compressed / expanded */
    .pipe(gulp.dest('./'));
}

function stylesCount() { return gulp.src('simple-modal.css').pipe(gulp_css_count()); }

/* Scripts ------------------------------------------------------------------ */

function scriptsLint() {
  var onError = function(err) { notify.onError({title: "JS"})(err); this.emit('end'); };
  return gulp.src('simple-modal.js')
    .pipe(plumber({errorHandler: onError}))
    .pipe(jshint())
    .pipe(jshint.reporter('default'))
    .pipe(jshint.reporter('fail'))
    .on('end', function() { logInfo('simple-modal.js'); });
}
...
function watchNow() { gulp.watch(['simple-modal.js', 'simple-modal.scss'], build); }
